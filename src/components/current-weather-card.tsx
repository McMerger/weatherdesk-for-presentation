...
export function CurrentWeatherCard({ data }: CurrentWeatherCardProps) {
  ...
  const windSpeedKmh = Math.round(data.windSpeedMps * 3.6);
      ...
      <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-8 items-center">
        <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
          <div className="flex items-start">
            <span className="text-7xl sm:text-8xl font-bold text-yellow-300 dark:text-yellow-200 drop-shadow-2xl">{temperature}</span>
